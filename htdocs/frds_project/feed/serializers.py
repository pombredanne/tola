...
class SiloSerializer(serializers.HyperlinkedModelSerializer):
	fields = serializers.HyperlinkedIdentityField(view_name='field_instance',format="html")
	class Meta:
		model = Silo  
...
class DataFieldSerializer(serializers.HyperlinkedModelSerializer):
    data = serializers.HyperlinkedIdentityField(view_name='value_instance', format='html')
    class Meta:
        model = DataField
